...
Page({
  ...
  onLoad: function (options) {
    ...
    that.getFirstData();
    that.getFirstScheduleData();  // 获取日程文章列表
    //登陆授权
  ...
  getFirstData: function () {
    let that = this;
    api.getFirstRecommendArticleList({
      query: {
        userId: wx.getStorageSync("userId"),
  ...
  getFirstScheduleData: function () {
    ...
    api.getFirstScheduleArticleList({
      ...
      success: (res) => {
        ...
        console.log(posts);
        this.setData({
          shedulePosts: this.data.posts.concat(posts),
          sheduleArticleId: res.data.retBean[res.data.retBean.length - 1].articleId,
          sheduleNow: new Date().getTime()
        });
      },
    });
  },
